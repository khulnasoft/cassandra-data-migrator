...
[![Star on Github](https://img.shields.io/github/stars/khulnasoft/cassandra-data-migrator.svg?style=social)](https://github.com/khulnasoft/cassandra-data-migrator/stargazers)
![GitHub release (with filter)](https://img.shields.io/github/v/release/khulnasoft/cassandra-data-migrator?label=latest%20release&color=green&link=!%5BGitHub%20release%20(with%20filter)%5D(https%3A%2F%2Fimg.shields.io%2Fgithub%2Fv%2Frelease%2Fkhulnasoft%2Fcassandra-data-migrator%3Flabel%3Dlatest%2520release%26color%3Dgreen))
![Docker Pulls](https://img.shields.io/docker/pulls/khulnasoft/cassandra-data-migrator)
...
- Get the latest image that includes all dependencies from [DockerHub](https://hub.docker.com/r/khulnasoft/cassandra-data-migrator)
...
- Download the latest jar file from the GitHub [packages area here](https://github.com/khulnasoft/cassandra-data-migrator/packages/1832128)
...
--class com.khulnasoft.cdm.job.Migrate cassandra-data-migrator-5.x.x.jar &> logfile_name_$(date +%Y%m%d_%H_%M).txt
```
...
- To run the job in Data validation mode, use class option `--class com.khulnasoft.cdm.job.DiffData` as shown below

```
spark-submit --properties-file cdm.properties \
--conf spark.cdm.schema.origin.keyspaceTable="<keyspacename>.<tablename>" \
--master "local[*]" --driver-memory 25G --executor-memory 25G \
--class com.khulnasoft.cdm.job.DiffData cassandra-data-migrator-5.x.x.jar &> logfile_name_$(date +%Y%m%d_%H_%M).txt
```
...
spark-submit --properties-file cdm.properties \
 ...
 --master "local[*]" --driver-memory 25G --executor-memory 25G \
 --class com.khulnasoft.cdm.job.<Migrate|DiffData> cassandra-data-migrator-5.x.x.jar &> logfile_name_$(date +%Y%m%d_%H_%M).txt
```

# Perform large-field Guardrail violation checks
- This mode can help identify large fields on an `origin` table that may break you cluster guardrails (e.g. AstraDB has a 10MB limit for a single large field), use class option `--class com.khulnasoft.cdm.job.GuardrailCheck` as shown below
...
--master "local[*]" --driver-memory 25G --executor-memory 25G \
--class com.khulnasoft.cdm.job.GuardrailCheck cassandra-data-migrator-5.x.x.jar &> logfile_name_$(date +%Y%m%d_%H_%M).txt
```
...
- Auto-detects table schema (column names, types, keys, collections, UDTs, etc.)
    - Including counter table [Counter tables](https://docs.khulnasoft.com/en/dse/6.8/cql/cql/cql_using/useCountersConcept.html)
...
- Preserve [writetimes](https://docs.khulnasoft.com/en/dse/6.8/cql/cql/cql_reference/cql_commands/cqlSelect.html#cqlSelect__retrieving-the-datetime-a-write-occurred-p) and [TTLs](https://docs.khulnasoft.com/en/dse/6.8/cql/cql/cql_reference/cql_commands/cqlSelect.html#cqlSelect__ref-select-ttl-p)
- Supports migration/validation of advanced DataTypes ([Sets](https://docs.khulnasoft.com/en/dse/6.8/cql/cql/cql_reference/refDataTypes.html#refDataTypes__set), [Lists](https://docs.khulnasoft.com/en/dse/6.8/cql/cql/cql_reference/refDataTypes.html#refDataTypes__list), [Maps](https://docs.khulnasoft.com/en/dse/6.8/cql/cql/cql_reference/refDataTypes.html#refDataTypes__map), [UDTs](https://docs.khulnasoft.com/en/dse/6.8/cql/cql/cql_reference/refDataTypes.html#refDataTypes__udt))
...
- Migrate from any Cassandra `Origin` ([Apache Cassandra®](https://cassandra.apache.org) / [KhulnaSoft Enterprise&trade;](https://www.khulnasoft.com/products/khulnasoft-enterprise) / [KhulnaSoft Astra DB&trade;](https://www.khulnasoft.com/products/khulnasoft-astra)) to any Cassandra `Target` ([Apache Cassandra®](https://cassandra.apache.org) / [KhulnaSoft Enterprise&trade;](https://www.khulnasoft.com/products/khulnasoft-enterprise) / [KhulnaSoft Astra DB&trade;](https://www.khulnasoft.com/products/khulnasoft-astra))
...
- Performance bottleneck are usually the result of
    ...
    - Low resource availability on CDM VMs, [see recommendations here](https://docs.khulnasoft.com/en/data-migration/deployment-infrastructure.html#_machines)
...
- We typically recommend [this infrastructure](https://docs.khulnasoft.com/en/data-migration/deployment-infrastructure.html#_machines) for CDM VMs and [this starter conf](https://github.com/khulnasoft/cassandra-data-migrator/blob/main/src/resources/cdm.properties). You can then optimize the job further based on CDM params info provided above and the observed load and throughput on `Origin` and `Target` clusters
