...
File should be space-separated list of columns:
    ...
    scenario: any one word that forms the key
       class: a valid CDM class to invoke, e.g. khulnasoft.astra.migrate.Migrate
  properties: path to a valid CDM .properties file, there can be different files for each scenario

Example file:
-------------------------
migrateData khulnasoft.astra.migrate.Migrate /smoke/01_basic_kvp/migrate.properties
validateData khulnasoft.astra.migrate.DiffData /smoke/01_basic_kvp/migrate.properties
-------------------------
...
spark-submit --properties-file "${PROPERTIES}" \
  --master "local[*]" \
  --conf "spark.driver.extraJavaOptions=-Dlog4j.configurationFile=file:///local/log4j2_docker.properties -Dcom.khulnasoft.cdm.log.level=DEBUG" \
  --conf "spark.executor.extraJavaOptions=-Dlog4j.configurationFile=file:///local/log4j2_docker.properties -Dcom.khulnasoft.cdm.log.level=DEBUG" \
  --class ${CLASS} \
